...
bool ahci_issue_command(AHCIDevice *device, int slot) {
  ...
  // TODO: There are probably all sorts of race conditions in this driver -- we probably need 
  // a per-port lock.
...
static PCIDeviceDriverError ahci_execute_command(PCIDeviceDriver *driver,
  ...
  switch ((enum AHCICommandIDs)command_id) {
    ...
    case AHCI_COMMAND_DEVICE_INFO:
    {
      ...
      struct AHCIDeviceInfoCommand *command = (struct AHCIDeviceInfoCommand *)input_buffer;
